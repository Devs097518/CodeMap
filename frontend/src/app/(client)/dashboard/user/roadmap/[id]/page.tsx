...
export default function RoadmapDetalheClientPage() {
  ...
  if (erro && !roadmap) {
    return <main className="min-h-screen bg-white px-4 sm:px-8 lg:px-16 py-10 font-sans text-red-500">{erro}</main>;
  }

  if (!roadmap) {
    return <main className="min-h-screen bg-white px-4 sm:px-8 lg:px-16 py-10 font-sans text-red-500">Roadmap não encontrado.</main>;
  }
      ...
      <div className="max-w-3xl mx-auto">
        ...
        <div className="flex flex-col gap-4">
          {topicos.length === 0 ? (
            ...
            topicos.map((topico) => (
              <div key={topico.id_topico} className="bg-white border-3 border-gray-300 shadow-[0px_0px_4px_3px_#00000010] rounded-2xl px-4 sm:px-6 py-5">
                <label className="flex items-center gap-2 text-sm text-gray-500 mb-2 cursor-pointer w-fit">
                  <input
                    type="checkbox"
                    checked={topico.estudado}
                    className="cursor-pointer w-3 h-3 bg-green-400 accent-emerald-600 rounded"
                    onChange={(e) => handleToggleTopico(topico.id_topico, e.target.checked)}
                  />
                  estudado
                </label>

                <h2 className="text-lg sm:text-xl font-semibold text-gray-900">{topico.titulo}</h2>
                ...
                {topico.subitens.length > 0 && (
                  <div className="border-l-3 border-gray-500 ml-2 pl-3 sm:pl-5 mt-4 flex flex-col gap-4">
                    {topico.subitens.map((sub) => (
                      <div key={sub.id_subitem} className="bg-gray-100 inset-shadow-sm rounded-2xl px-4 sm:px-5 py-4">
                        <label className="flex items-center gap-2 text-sm text-gray-500 mb-2 cursor-pointer w-fit">
                          <input
                            type="checkbox"
                        ...
                        <h3 className="text-lg sm:text-xl font-semibold text-gray-900">{sub.titulo}</h3>
