...
export default function FoldersPage() {
  ...
  const handleCriar = () => {
    ...
    if (!trimmed) return;
    const nova: Pasta = { id: crypto.randomUUID(), titulo: trimmed };
  ...
  return (
    <main className="min-h-screen bg-white px-10 py-10 font-sans">
      ...
      <div className="mb-10">
        ...
        <p className="text-sm text-gray-500 mt-0.5">continue as suas notas</p>
      </div>
      ...
      <div className="mb-10">
        ...
        <div className="flex items-center gap-0 w-64 bg-[#eeff66] rounded-xl overflow-hidden px-4 py-3">
          <input
            ...
            placeholder="Título"
            className="flex-1 bg-transparent text-sm font-semibold text-gray-800 placeholder-gray-600 outline-none"
          />
          <button
            onClick={handleCriar}
        ...
        {pastas.length === 0 ? (
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-4">
            {pastas.map((pasta) => (
              <button
                key={pasta.id}
                className="group flex items-center justify-between bg-[#eeff66] rounded-2xl px-5 py-5 text-left hover:bg-[#d4e600] active:scale-95 transition-all duration-150 cursor-pointer"
              >
